...
function buttonSearchTitle() {
  ...
  const isSelected = element.classList.contains("hover:bg-amber-50");

  if (isSelected) {
    setSearchType("title");
    element.classList.remove("hover:bg-amber-50");
    element.classList.add("bg-amber-50", "text-black", "hover:bg-primary-blackrock");
  } else {
    setSearchType("");
    element.classList.remove("bg-amber-50", "text-black", "hover:bg-primary-blackrock");
    element.classList.add("hover:bg-amber-50");
  }
}
      ...
      <div className='outer-search-bar-container p-[20px] max-w-full min-h-[20px] flex justify-between gap-2 '>
        ...
        <div className='flex gap-2'>
          ...
          onClick={() => setSearchType("author")}>Search by Author</button>
            <div className='inner-search flex flex-row gap-2 items-center rounded-4xl p-3 bg-amber-50 h-[70px]'>
              <input
