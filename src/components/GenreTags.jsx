...
const GenreTags = ({ genre }) => {
  // genre is an array of subjects/genre
      ...
      {genreArray.length > 0 ? (
        genreArray.map((genreName, index) => {
          ...
          return (
            <div
              key={index}
              className={`${lowerCasedGenre}-tag ${tagStyle} rounded-2xl p-2 text-[12px] font-satoshi max-h-[40px] shadow-custom4-first-content text-center max-w-[130px] flex items-center`}
            >
              {genreName}
            </div>
          );
        ...
        <div
          className={`unkown-tag bg-[#bfbfbf] rounded-2xl p-2 text-[12px] font-satoshi max-h-[36px] shadow-custom4-first-content text-center w-[130px]`}
        >
          Unknown Genre
        </div>
      )}
